...
FileDelete($fCSVo)
FileDelete($fError)
Local $tHelp = 'Use: SteamServerQuery {options} IP:port' & @CRLF & '  or: SteamServerQuery {options} URL:port' & @CRLF & @CRLF & '-h = Displays this help text' & @CRLF & '-pw = Print output to commandline with labels' & @CRLF & '-po = Print output to commandline WITHOUT labels' & @CRLF & '-tw = Write output to txt file with labels to [' & $fTXTw & ']' & @CRLF & '-to = Write output to txt file WITHOUT labels to [' & $fTXTo & ']' & @CRLF & '-cw = Write output to csv file with Labels to [' & $fCSVw & ']' & @CRLF & '-co = Write output to csv file WITHOUT labels to [' & $fCSVo & ']' & @CRLF & @CRLF & 'Example 1: SteamServerQuery -pw -co 127.0.0.1:26500' & @CRLF & 'Example 2: SteamServerQuery -tw phoenix125.com:26500' & @CRLF & @CRLF & 'NOTICE! For many servers, use Query Port +1' & @CRLF & '   Ex: If query port is 30000, use 30001' & @CRLF & @CRLF & 'Thank you. Visit https://github.com/phoenix125 for updates and/or more programs.' & @CRLF
If $CmdLine[0] < 1 Then
ConsoleWrite($tHelp)
Exit
EndIf
...
Next
EndIf
If Not $bFinalBreak Then $sString = StringTrimRight($sString, StringLen($sNewLine))
Return $sString
EndFunc
